...
// 一条永远连不通的 L4 路由。
function generateSshRouteBlock() {
  if ((process.env.SSH_ENABLED || 'false') !== 'true') return '';
  if (!process.env.SSH_AUTHORIZED_KEYS) return '';
...
function generateCaddyConfig(l4Block, httpBlock, panelSni) {
  ...
  content = content
    ...
    .replace(/\$\{SSH_ROUTE_BLOCK\}/g, generateSshRouteBlock())
