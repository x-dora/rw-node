...
function extractInboundConfig(config) {
  ...
  return { realityRoutes: mergedReality, httpRoutes, conflicts };
}
...
function generateL4RouteBlock(realityRoutes) {
  if (realityRoutes.length === 0) return '';
  const lines = [];
...
function generateCaddyConfig(l4Block, httpBlock) {
...
async function main(configPath) {
  ...
  while (true) {
    ...
    const { realityRoutes, httpRoutes, conflicts } = extractInboundConfig(configJson);

    const hashInput = JSON.stringify({ realityRoutes, httpRoutes });
    ...
    const l4Block = generateL4RouteBlock(realityRoutes);
    const httpBlock = generateHttpRouteBlock(httpRoutes);

    fs.writeFileSync(configPath, generateCaddyConfig(l4Block, httpBlock));
    caddyFmt(configPath);
