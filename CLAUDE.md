...
- **三种部署方式**：
  ...
  - PaaS + FRP 反向 TCP 隧道镜像（`ghcr.io/x-dora/rw-node:latest-paas-frp`，用于 PaaS 不能公开自定义 TCP 入站端口的场景）
...
- `docker/Dockerfile.paas-frp` — PaaS 反向 TCP 隧道版镜像（内置 frpc，发布为 `:latest-paas-frp` / `:<version>-paas-frp`）
- `docker/docker-entrypoint.sh` / `docker-entrypoint.official.sh` — 标准容器入口脚本
- `docker/docker-entrypoint.paas-frp.sh` — PaaS FRP 入口脚本，启动 rw-node、生成 `frpc.toml`、等待本地 HTTPS 就绪后启动 frpc
...
- `PORT` — 某些 PaaS 强制要求的 HTTP 健康检查端口；PaaS FRP 入口脚本会在该端口启动辅助 HTTP health server，不参与节点流量
